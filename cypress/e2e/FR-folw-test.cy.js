...
describe('FR-flow-test', () => {
    it('completes FR flow', () => {
      ...
      // Upload image
      cy.get('input[type="file"]').selectFile('cypress/fixtures/images/FR-test.jpg', {
        force: true,
      }).wait(7000)

      //slecting service
      cy.get('#v5-services-page-AI Furniture Restyle').click()
  
      // selecting space drop down
      cy.contains('button', 'Select').find('span.i-tabler\\:chevron-down').click()

      //selecting space
      cy.get('#v5-tool-furniture-restyle-space-bedroom').click()

      //selecting style drop down
      cy.contains('button', 'Modern').find('span.i-tabler\\:chevron-down').click()
  
      //selecting style
      cy.get('#v5-tool-furniture-restyle-style-contemporary').click()
  
      //generate button click
      cy.get('#v5-tool-furniture-restyle-generate-button').click()
      ...
      //regenerate flow
      cy.contains('AI Furniture Restyle').click()
  
      // selecting space drop down
      cy.contains('button', 'Select').find('span.i-tabler\\:chevron-down').click().wait(1000)
  
      //selecting space
      cy.get('#v5-tool-furniture-restyle-space-office').click()
  
      // selecting style
      cy.get('button').contains('Modern').click()
  
      //selecting style 
      cy.get('#v5-tool-furniture-restyle-style-scandinavian').click()
  
      //generate button click
      cy.get('#v5-tool-furniture-restyle-generate-button').click().wait(30000)
