/**
 * FC-flow-test (Optimized)
 *
 * Test Suite  : Floor Change (FC) end-to-end flow
 * Coverage    : Login → Upload → Service → Gen-1 → Download (Normal + Upscale) →
 *               Feedback → Bookmark → Gen-2 (Dining Room + Brown Marble)
 *
 * Helpers keep selectors DRY; flow and waits match the working FC-flow-test.cy.js.
 */
...
describe('FC-flow-test', () => {
  it('completes FC flow: login → upload → gen-1 → download (normal + upscale) → feedback → bookmark → gen-2', () => {
    // ── 1. Visit site ─────────────────────────────────────────────────────────
    ...
    // ── 5. Select Floor Change service ──────────────────────────────────────────
    selectFCService()

    // ── 6. Gen-1 config: Kitchen + Cream Marble ─────────────────────────────────
    dismissBlockingModals()
    selectSpace(SEL.spaceKitchen, 1000)
    selectMaterial(SEL.materialCreamMarble)
    ...
    // ── 11. Gen-2: Dining Room + Brown Marble ───────────────────────────────────
    openFCSidebar()
    selectSpace(SEL.spaceDiningRoom, 1000)
    selectMaterial(SEL.materialBrownMarble)
    clickGenerate()
    cy.wait(30000)
  })
})
