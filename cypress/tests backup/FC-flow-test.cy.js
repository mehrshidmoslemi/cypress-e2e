...
describe('FC-flow-test', () => {
    it('completes FC flow', () => {
      // Visit site
      ...
      // Upload image
      cy.get('input[type="file"]').selectFile('cypress/fixtures/images/FC-test.jpg', {
        force: true,
      }).wait(7000)

      //slecting service
      cy.get('#v5-services-page-AI Floor Change').click()
  
      // selecting space drop down
      cy.contains('button', 'Select').find('span.i-tabler\\:chevron-down').click()

      //selecting space
      cy.get('#v5-tool-floor-change-space-kitchen').click()

      //selecting style drop down
      cy.contains('button', 'White Marble').find('span.i-tabler\\:chevron-down').click()
  
      //selecting style
      cy.get('#v5-tool-floor-change-material-cream-marble').click()
  
      //generate button click
      cy.get('#v5-tool-floor-change-generate-button').click()
      ...
      //regenerate flow
      cy.contains('AI Floor Change').click()
  
      // selecting space drop down
      cy.contains('button', 'Select').find('span.i-tabler\\:chevron-down').click().wait(1000)
  
      //selecting space
      cy.get('#v5-tool-floor-change-space-dining-room').click()
  
      // selecting style
      cy.get('button').contains('White Marble').click()
  
      //selecting style 
      cy.get('#v5-tool-floor-change-material-brown-marble').click()
  
      //generate button click
      cy.get('#v5-tool-floor-change-generate-button').click().wait(30000)
